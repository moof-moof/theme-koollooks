...
namespace eval ttk::theme::koollooks {
    ...
    array set I [LoadImages \
                     [file join [file dirname [info script]] koollooks] *.gif]
...
#   ------------ Defines CELL images (not indicator images)! -----------
...
#   ------------ Defining the INDICATOR images: ------------------------

		## Possible states: selected, focus, user1 (open), user2 (leaf).
		
		# __STATE_____________________NAME__________LEGEND__________
		# 							*-right-n 		"Normal"
		# {!user2 !user1 selected} 	*-right-sn 		Closed + Selected
		# {user1 selected} 			*-down-sn 		Open + Select
		# user1 					*-down-n 		Open + NoSelect
		# user2 					  empty 		No arrow


		ttk::style element create Treeitem.indicator image \
			[list 							$I(tree-arrow-right-n) \
				{!user2 !user1 selected} 	$I(tree-arrow-right-sn) \
				{user1 selected} 			$I(tree-arrow-down-sn) \
				user1 						$I(tree-arrow-down-n) \
				user2 						$I(empty)] \
			-sticky w -width 15
		   

		## Menu-bar     (Configure this in application-code instead)  
		#
		#ttk::style element create Menuitem.indicator image \
			#[list 			$I(sub-menu-indicator-n) \
				#selected 	$I(sub-menu-indicator-sn) \
				#active	 	$I(sub-menu-indicator-sn) \
				#disabled 	$I(empty) \
				#user2 		$I(empty) \
			#] \
			#-sticky e -width 15
...
#        ttk::style configure Menuitem.indicatoron 0 -indicatoron 0
        ...
        ttk::style element create dflt.button image \
            [list $I(button-nx) \
                 pressed $I(button-px) \
                 selected $I(button-px) \
                 active $I(button-ax) \
                 disabled $I(button-dx) \
                ] \
            -border 4 -sticky ew
...
#########################################################################################################
		...
		ttk::style configure TNotebook.Tab -padding {0 1 0 0} -anchor center
        ttk::style map TNotebook.Tab \
        -padding [list selected {0 0 0 0}] \
        -foreground [list !selected  $colors(-disabledfg)]
        ...
        foreach direc {up down left right} {
            ttk::style element create ${direc}arrow image \
                [list $I(arrow${direc}-n) \
                     disabled $I(arrow${direc}-d) \
                     pressed $I(arrow${direc}-p) \
                     active $I(arrow${direc}-a)] \
                -border 1 -sticky {}
        }
